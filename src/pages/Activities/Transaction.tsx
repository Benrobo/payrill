...
function TransactionActivities() {
  const { Data, Loader, walletInfo, Error, setData, setLoader, setError } =
  ...
  async function fetchTransactions() {
    try {
      ...
      setError((prev: any) => ({
        ...prev,
        transactions: ` An Error occured: ${e.message} `,
      }));
    }
  }
    ...
    <Layout sideBarActiveName="activities">
      <div className="w-full h-screen flex flex-col items-center justify-start">
        ...
        >
          ...
          <span className="text-white-100 font-extrabold bg-blue-300 rounded-md px-2 py-1 text-[10px] ">
...
function TransactionLists({ handleActiveState }: any) {
  ...
  function determineIdentity(
    ...
    if (receiver_id !== id && sender_id === id) {
      (_result.toMe = false), (_result.fromMe = true);
      _result.type = type;
    }
    return _result;
  }
  ...
  return (
    <div
      id="transaction-list"
      className="w-full px-2 md:px-0 flex flex-col items-center justify-center gap-5"
    >
      {Data.transactions.length > 0
        ? Data.transactions.map((tra: any) => (
            <div
              ...
              data-id={tra.id}
              key={tra.id}
              className="w-full flex flex-row items-row justify-between bg-dark-200 p-4 rounded-[12px] cursor-pointer relative"
              onClick={(e) => {
                handleActiveState("viewTransaction", e);
              }}
            >
              <button className="w-auto absolute top-0 right-0 text-[12px] text-white-200 px-2 py-1 bng-blue-300 rounded-md " data-id={tra.id} onClick={(e) => {
                handleActiveState("viewTransaction", e);
              }}>
                View
              </button>
              ...
              >
                ...
                ></div> */}
                {determineIdentity(tra.receiver_id, tra.sender_id, tra.type)
                  .toMe &&
                  determineIdentity(tra.receiver_id, tra.sender_id, tra.type)
                    .type === "deposit" &&
                  !determineIdentity(tra.receiver_id, tra.sender_id, tra.type)
...
function ViewTransaction({
  ...
  function filterTransaction() {
    const filtered = Data.transactions.filter(
      (data: any) => data.id === transactionId
    )[0];
  ...
  function filterUser(sId: string, rId: string, type: string) {
    ...
    if (rId === id && sId !== id) {
      _result.toMe = true;
      _result.fromMe = false;
      _result.sName = localUsers.filter(
        (user: any) => user.id === sId
      )[0]?.name;
      _result.rName = localUsers.filter(
        (user: any) => user.id === rId
      )[0]?.name;
      _result.sUname = localUsers.filter(
        (user: any) => user.id === sId
      )[0]?.username;
      _result.rUname = localUsers.filter(
        (user: any) => user.id === rId
      )[0]?.username;
      _result.type = type;
    }
    ...
    if (rId !== id && sId === id) {
      (_result.toMe = false), (_result.fromMe = true);
      _result.sName = Data.users.filter(
        (user: any) => user.id === sId
      )[0]?.name;
      _result.rName = Data.users.filter(
        (user: any) => user.id === rId
      )[0]?.name;
      _result.sUname = Data.users.filter(
        (user: any) => user.id === sId
      )[0]?.username;
      _result.rUname = Data.users.filter(
        (user: any) => user.id === rId
      )[0]?.username;
      _result.type = type;
    }
    ...
    >
      <div className="w-full flex flex-row items-row justify-between mt-3 px-3">
        <button
          ...
          onClick={() =>
            setActiveState((prev: any) => ({ viewTransaction: false }))
          }
        >
      ...
      >
        {selectedData.user.fromMe && (
          <div
            id="img"
            className="w-[70px] h-[70px] rounded-[50%] bg-dark-100 border-[3px] border-solid border-blue-200 "
            style={{
              ...avatarImg,
              backgroundImage: `url("https://avatars.dicebear.com/api/avataaars/${selectedData.user.rUname}.svg")`,
            }}
          ></div>
        )}

        {selectedData.user.toMe && (
          <div
            id="img"
            className="w-[70px] h-[70px] rounded-[50%] bg-dark-100 border-[3px] border-solid border-blue-200 "
            style={{
              ...avatarImg,
              backgroundImage: `url("https://avatars.dicebear.com/api/avataaars/${selectedData.user.sUname}.svg")`,
            }}
          ></div>
        )}
        ...
        <p className="text-white-300 text-[12px] pb-4 mt-2">
          {selectedData.transactions.title}
        </p>
        <p className="text-white-100 ">
          You
          {selectedData.user.fromMe &&
            selectedData.user.type === "transfer" &&
            " sent "}
          {selectedData.user.toMe &&
            selectedData.user.type === "transfer" &&
            " received "}
          {selectedData.user.toMe &&
            selectedData.user.type === "deposit" &&
            " deposited "}
          {selectedData.user.toMe &&
            selectedData.user.type === "crypto" &&
            " purchased "}
          <span className="font-extrabold">
            {formatCurrency(
              selectedData.transactions.currency,
              selectedData.transactions.amount
            )}
          </span>
          {selectedData.user.fromMe &&
            selectedData.user.type === "transfer" &&
      ...
      >
        <div className="w-full flex flex-row items-center justify-between gap-3 mt-10 px-6 ">
          ...
          <p className="text-white-300 text-[12px] pb-4">
            {moment(selectedData.transactions.createdAt).format(
              "MMM Do YY h:mm a"
            )}
          </p>
        </div>
